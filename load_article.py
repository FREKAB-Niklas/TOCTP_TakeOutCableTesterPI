...
def run_article(filename, description, rev, pins, spacing, length, width, inner_diameter, takeouts):
    # Get the directory where the script is located
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def load_article_file():
    ...
    rev = [line.strip().split('=')[1] for line in lines if line.startswith("rev")][0]
    ...
    run_article(filename, description, rev, pins, spacing, length, width, inner_diameter, takeouts)
